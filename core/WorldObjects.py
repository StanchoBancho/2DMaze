...
class World:
    ...
    def is_position_free(self, position):
        is_widht_fine = position[0] >= 0 and position[0] < self.maze.size[0]
        is_height_fine = position[1] >= 0 and position[1] < self.maze.size[1]
        if not (is_widht_fine and is_height_fine):
            return False
...
class Maze:
    ...
    def __init__(self, size):
        ...
        #0  - empty
        #1  - black
        self.board = [[0 for j in range(self.height)]
    ...
    def make_random_puzzle(self):
        ...
        while len(self.possible_moves) > 0:
            ...
            chosen_square = self.possible_moves[index]
